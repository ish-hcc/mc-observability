...
> 최대 59분 전에 만들어진 데이터를 받을 수 있었습니다.
...
요청 도착
   │
   ├─ 캐시에 있고 아직 신선함        → 그대로 반환                   (hitFresh)
   │
   ├─ 캐시에 있지만 신선도가 지났음  → 일단 그대로 반환하고,          (hitStale)
   │                                  백그라운드에서 새로 읽어 교체
   │
   └─ 캐시에 없음                    → 지금 읽어서 반환               (missCold)
```
...
**청크 캐싱**은 이번 범위에 넣지 않았습니다.

시계열의 과거 구간은 변하지 않으므로, "최근 1시간"을 60개의 1분 조각으로 나눠 캐싱하면
1분이 지날 때 **새로 읽어야 할 구간이 1분뿐**입니다. 지금은 1분이 지나면 60분을 통째로 다시
읽습니다. 이론상 InfluxDB 조회량이 크게 줄어드는 가장 큰 개선이지만, 상대 구간(`range=1h`)을
절대 시각 구간으로 바꾸는 쿼리 빌더 수정이 필요하고 다운샘플 라우팅·retention policy 처리와
얽혀 있어 별도 작업으로 분리했습니다.
